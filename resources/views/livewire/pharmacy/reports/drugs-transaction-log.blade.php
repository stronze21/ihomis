                ...
                <tr classs="border border-black">
                    <td class="text-xs border border-black">
                        <div class="flex flex-col">
                            ...
                            <div class="ml-10 text-xs text-slate-800">{{$log->drug->dmdnost}}{{$log->drug->strength->stredesc ?? ''}} {{$log->drug->form->formdesc ?? ''}}</div>
                        </div>
                    </td>
                    <td class="text-xs text-right border border-black">{{number_format($log->beg_bal)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->beg_bal * $log->current_price->dmduprice,2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->purchased)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->purchased * $log->current_price->dmduprice,2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->available())}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->current_price->dmduprice,2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->available_amount(),2)}}</td>
                    ...
                    <td class="text-xs text-right border border-black">{{number_format($log->current_price->dmselprice,2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->total_sales(), 2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->total_cogs(), 2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->total_profit(), 2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->ending_balance(), 2)}}</td>
                    <td class="text-xs text-right border border-black">{{number_format($log->ending_balance() * $log->current_price->dmduprice, 2)}}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="25" class="font-bold text-center uppercase bg-red-400 border border-black">No record found!</td>
                </tr>
                @endforelse
            </tbody>
        </table>
