            ...
            <div class="flex flex-col max-h-screen p-1 overflow-scroll">
                <div class="flex justify-between my-3">
                    ...
                    <div class="flex ml-auto">
                        <div><button class="ml-2 btn btn-sm btn-warning" onclick="charge_items()" wire:loading.attr="disabled" wire:loading.class="btn-secondary">Charge Slip</button></div>
                        <div><button class="ml-2 btn btn-sm btn-primary" onclick="issue_order()" wire:loading.attr="disabled" wire:loading.class="btn-secondary">Issue</button></div>
                ...
                <table class="w-full mb-40 text-sm table-compact">
                    <thead class="sticky top-0 font-bold bg-gray-200">
                        <tr>
                            <td colspan="3" class="w-1/3 border border-black"><span>Hospital #: </span> <span class="fw-bold">{{$encounter->patient->hpercode}}</span></td>
                        ...
                        <tr>
                            <td colspan="3" class="w-1/3 border border-black"><span>Last Name: </span> <span class="fw-bold">{{$encounter->patient->patlast}}</span></td>
                            <td colspan="3" class="w-1/3 border border-black"><span>First Name: </span> <span class="fw-bold">{{$encounter->patient->patfirst}}</span></td>
                            <td colspan="3" class="w-1/3 border border-black"><span>Middle Name: </span> <span class="fw-bold">{{$encounter->patient->patmiddle}}</span></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="w-1/3 border border-black"><span>Room/Encounter Type: </span> <span class="fw-bold">{{$encounter->toecode}}</span></td>
                            <td colspan="6" class="border border-black"><span>Encounter Date/Time: </span> <span class="fw-bold">{{\Carbon\Carbon::create($encounter->encdate)->format('F j, Y / g:i A')}}</span></td>
                        </tr>
                        <tr class="border border-black">
                            ...
                            <td></td>
                            <td></td>
                            <td colspan="2" class="text-right uppercase">Grand Total: {{number_format($encounter->rxo->sum('pcchrgamt', 2))}}</td>
                        </tr>
                        <tr class="border border-black">
                            ...
                            <td class="w-20 text-right"><div class="tooltip" data-tip="Quantity Ordered">Q.O.</div></td>
                            <td class="w-20 text-right"><div class="tooltip" data-tip="Quantity Returned">Q.R.</div></td>
                            <td class="w-20 text-right"><div class="tooltip" data-tip="Quantity Issued">Q.I.</div></td>
                    ...
                    <tbody class="bg-white">
                        ...
                        <tr class="border">
                            <td class="whitespace-nowrap w-min" title="View Charge Slip"> @if($rxo->pcchrgcod) <a class="font-semibold text-blue-600" href="{{route('dispensing.rxo.chargeslip', $rxo->pcchrgcod)}}" target="_blank">{{$rxo->pcchrgcod}}</a> @endif</td>
                            <td class="align-top whitespace-nowrap w-min">
                            ...
                            <td class="w-max">
                                <div class="flex flex-col">
                                    ...
                                    <div class="text-xs text-center text-slate-800">{{$rxo->dm->dmdnost}}{{$rxo->dm->strength->stredesc ?? ''}} {{$rxo->dm->form->formdesc ?? ''}}</div>
                                </div>
                            </td>
                            ...
                            <td class="w-20 text-right" title="Return Issued">
                                ...
                                <span class="cursor-pointer" onclick="return_issued('{{$rxo->docointkey}}', '{{$rxo->dm->generic->gendesc}} <br>{{$rxo->dmdnost}} {{$rxo->dm->strecode}}, {{$rxo->dm->form->formdesc}}', {{$rxo->pchrgup}}, {{$rxo->qtyissued}})">
        ...
        <div class="col-span-4 xl:col-span-1">
            <div class="overflow-auto max-h-96">
                <div class="flex justify-between">
                    ...
                    <div class="w-1/2 p-2">
                        <input type="text" placeholder="Type here" class="w-full input input-sm input-bordered" wire:model.lazy="generic" />
                    </div>
                </div>
                <table class="table w-full table-compact">
                    ...
                    <tbody>
                        ...
                        <tr class="cursor-pointer hover" onclick="select_item('{{$stock->id}}', '{{$stock->drug->generic->gendesc}}', '{{$stock->current_price->dmselprice}}')">
                            <td>
                                <div class="flex flex-col">
                                    <div class="text-xs text-slate-600">{{$stock->charge->chrgdesc}}</div>
                                    <div class="font-bold">{{$stock->drug->generic->gendesc}}</div>
                                    <div class="text-xs text-center text-slate-800">{{$stock->drug->dmdnost}}{{$stock->drug->strength->stredesc}} {{$stock->drug->form->formdesc}}</div>
                                </div>
                            </td>
                            <td class="text-right">
                                <div class="flex flex-col">
            ...
            Swal.fire({
                html: `
                        ...
                        <div class="w-full mb-3 form-control">
                            ...
                            <input id="order_qty" type="number" value="1" class="w-full input input-bordered" />
                        </div>
