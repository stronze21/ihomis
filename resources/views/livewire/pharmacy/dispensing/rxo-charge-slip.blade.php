        ...
        <div class="p-2">
            <div class="flex flex-col text-xs/4">
                ...
                <div class="flex flex-col text-left whitespace-nowrap">
                    <div>Dep't./Section: <span class="font-semibold">{{$prescription->employee->dept->deptname ?? ''}}</span></div>
                    <div>Date/Time: <span class="font-semibold">{{date('F j, Y h:i A' , strtotime($rxo_header->dodate))}}</span></div>
                    ...
                    <div>Ward: <span class="font-semibold">{{$prescription ? $prescription->adm_pat_room->ward->wardname : ''}}</span></div>
                    <div>Ordering Physician: <span class="font-semibold">{{$prescription ? 'Dr. '.$prescription->employee->fullname() : ''}}</span></div>
                </div>
            </div>
                ...
                <tbody>
                    @foreach($rxo as $item)
                        <tr class="border-t border-black border-x">
                            <td class="font-semibold text-wrap" colspan="4">{{ $item->dm->drug_name() }}</td>
                        </tr>
            ...
            <div class="flex flex-col py-0 my-0 text-left text-xs/4 whitespace-nowrap">
                ...
                <div class="mt-10 text-right justify-content-end"><span class="border-t border-black">Signature Over Printed Name</span></div>
            </div>
        </div>
    </div>
    {{-- @if($print)
