...
<div class="flex flex-col p-5 mx-auto mt-5 max-w-7xl">
    <div class="p-4 mb-3 bg-white rounded-lg">
        <div class="flex justify-between">
            ...
            <div>
                    <button class="btn btn-sm btn-warning" onclick="save_lock()" wire:loading.attr="disabled">Save & Lock</button>
            </div>
            <div>
    ...
    <div class="flex flex-col p-5 bg-white rounded-lg">
        <div class="flex justify-between w-full pb-2 border-b">
            <div class="flex flex-col w-1/2">
                <div class="flex"><div class="w-36">Delivery Date:</div> <div class="font-bold uppercase w-96">{{$details->delivery_date}}</div></div>
                <div class="flex"><div class="w-36"> Supplier:</div> <div class="font-bold uppercase w-96">{{$details->supplier->suppname}}</div></div>
                <div class="flex"><div class="w-36"> Source of Fund:</div> <div class="font-bold uppercase w-96">{{$details->charge->chrgdesc}}</div></div>
            </div>
            <div class="flex flex-col w-1/2">
                <div class="flex"><div class="w-36"> Purchase Order #:</div> <div class="font-bold uppercase w-96">{{$details->po_no}}</div></div>
                <div class="flex"><div class="w-36"> Sales Invoice #:</div> <div class="font-bold uppercase w-96">{{$details->si_no}}</div></div>
                <div class="flex"><div class="w-36"> Status:</div>
                    <div class="font-bold uppercase w-96">
                        <div class="badge @if($details->status == 'pending') badge-ghost @else badge-success @endif">{{$details->status}}</div>
                    </div>
                </div>
            </div>
        </div>
        <table class="table w-full mt-3 table-compact">
            ...
            <tbody class="border">
                ...
                @php
                    $dm = $item->drug;
                @endphp
                <tr @if($details->status == 'pending') class="cursor-pointer hover" onclick="edit_item('{{$item->id}}', '{{$item->lot_no}}', '{{$item->qty}}', '{{$item->unit_price}}', '{{$item->markup_price}}', '{{$item->total_amount}}', '{{$item->expiry_date}}', '{{$dm->drug_name()}}')" @endif>
                    <td>{{$item->lot_no}}</td>
...
<script>
    ...
    function edit_item(item_id, item_lot_no, item_qty, item_unit_price, item_markup_price, item_total_amount, item_expiry_date, drug_name)
    {
        Swal.fire({
            html: `
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                html: `<span class="text-xl font-bold uppercase"> Remove Item: <br>`+drug_name+` </span>`,
                icon: 'error',
                showCancelButton: true,
                showConfirmButton: true,
